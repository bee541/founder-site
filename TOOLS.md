...
Whop SDK is installed (`@whop/sdk` global). Credentials stored at `/root/.config/whop/.env` (chmod 600).

- **CLI wrapper:** `whop` — runs Node one-liners with auto-loaded credentials
- **Credentials:**
  - `WHOP_API_KEY=apik_XyS8gGapXphPy_C…`
  - `WHOP_COMPANY_ID=biz_CR7TgUn4i5qg8z` (company: **SideHustler**)

**Verified permissions:**
- ✅ `products.list`
- ✅ `invoices.list`
- ✅ `paymentMethods.list`
- ❌ `company:basic:read` — missing key permission; cannot read company metadata directly
- ❌ `dms:read` — cannot access DM data
- ✓ `user` owner identified as **SideHustler** (`user_OlxUvEHxL6kOj`, `perception101@whop.com`)

**Known product (as of last check):**
- `prod_zoqOMfCW4EWyO` — *Viral Content Starter Kit*

**Usage:** `whop` runs product list by default. Pass any Node JS to e.g. query invoices, list memberships, check payment history. Reference the SDK docs at `https://docs.whop.com/developer/api`.
